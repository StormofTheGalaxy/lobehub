...
export class ConnectorModel {
  ...
  /**
   * Same lookup as {@link queryByIdentifiers}, but ignoring the workspace scope:
   * matches every base row owned by the user regardless of `workspace_id`. Used
   * only to recover from a unique-violation on bootstrap sync, where the row
   * blocking the insert may be a legacy personal-scoped one.
   */
