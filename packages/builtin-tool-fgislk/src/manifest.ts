...
const reportSchema = {
  ...
  properties: {
    attachments: {
      description:
        'Вложения отчёта: фотофиксация, акты, паспорта посадочного материала. ' +
        'Идентификаторы и контрольные суммы MD5 инструмент проставит сам по байтам файла.',
      items: {
        additionalProperties: false,
        properties: {
          ...
          file: {
            description:
              'Имя файла ровно как он называется у пользователя, например «IMG_2465.jpg». ' +
              'Ищется среди файлов, приложенных к переписке, и в каталоге filesDir на сервере. ' +
              'Если пользователь приложил фото к сообщению — просто укажи его имя.',
            type: 'string',
          },
        },
        required: ['file'],
  ...
  api: [
    ...
    },
    {
      description:
        ...
        'XML формируется по официальной XSD-схеме Рослесхоза и вместе с вложениями ' +
        'отдаётся пользователю ZIP-пакетом — только если он прошёл валидацию. Коды строк и единицы ' +
        'измерения подставляются из формы приказа № 112, контрольные суммы вложений ' +
        'считаются по файлам. Не пересказывай содержимое XML и не придумывай sandbox-ссылку: ' +
        'пользователь скачает пакет настоящей кнопкой результата.',
      ...
      parameters: {
        ...
        properties: {
          filesDir: {
            description: 'Каталог с файлами вложений на сервере (если вложения лежат там)',
            type: 'string',
          },
          outputPath: {
            description:
              'Необязательно: путь для дополнительной записи файла на диск сервера. ' +
              'Обычно не нужен — готовый документ и так отдаётся ссылкой на скачивание.',
            type: 'string',
          },
          report: reportSchema,
        },
    ...
    },
    {
      description:
        ...
        'после чего документ проверяется и отдаётся файлом на скачивание. Нужно для отчётов, ' +
        'сделанных по устаревшей схеме.',
      humanIntervention: 'always',
