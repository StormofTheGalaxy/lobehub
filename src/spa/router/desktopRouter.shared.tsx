...
const createMainAreaChildrenDefinition = (options: MainAreaRouteOptions = {}): RouteObject[] => [
  ...
  // Fork-only main-area routes. Upstream extracted this tree into
  // `desktopRouter.shared.tsx`, so they move here from the old config files —
  // the Business* spreads above are upstream's extension points and do not
  // cover these two.
  {
    element: dynamicElement(
  ...
  {
    element: dynamicElement(() => import('@/routes/invite/[token]'), 'Desktop > Invite'),
    errorElement: <ErrorBoundary />,
    path: 'invite/:token',
  },
