...
async function upsertConnectorEntry(
  ...
  try {
    ...
    // A concurrent bootstrap (or a legacy row created before workspace scoping)
    // already owns this identifier — reuse it instead of failing the sync.
    const duplicate = await ctx.connectorModel.queryByIdentifiers([params.identifier]);
    if (duplicate[0]) {
      await ctx.connectorModel.update(duplicate[0].id, { metadata });
      return { connectorId: duplicate[0].id, writable: true };
    }

    const legacyDuplicate = await ctx.connectorModel.queryByIdentifiersAnyScope([
      params.identifier,
    ]);
    if (legacyDuplicate[0]) {
      await ctx.connectorModel.update(legacyDuplicate[0].id, { metadata });
      return { connectorId: legacyDuplicate[0].id, writable: true };
    }
